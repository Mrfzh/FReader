...
public class FemaleModel implements IFemaleContract.Model {
    ...
    private List<String> mRankNameList = Constant.FEMALE_HOT_RANK_NAME;            // 排行榜名称
    ...
    public void getHotRankData() {
        ...
        // 开启多个线程进行请求
        mCountDownLatch = new CountDownLatch(Constant.FEMALE_HOT_RANK_NUM);
        mReturnMsg = "";
        for (int i = 0; i < Constant.FEMALE_HOT_RANK_NUM; i++) {
            request(mNovelInfoList.get(i), UrlObtainer.getRankNovels(Constant.FEMALE_HOT_RANK_ID.get(i)));
        }
        // 等待请求线程完成
